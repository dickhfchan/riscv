//! Ferrite OS cspace_a — Phase 13: per-process CSpace.
//! Slot 1 in this process's private CSpace is a UntypedMemory cap.
//! Retype from it → expect OK → demonstrates CSpace-scoped allocation.
...
const SLOT_PRIV_UT:     usize = 1;  // private UntypedMemory (only in A's CSpace)
const SLOT_NEW_EP:      usize = 2;  // destination for retyped Endpoint
...
pub unsafe extern "C" fn _start() -> ! {
    // Attempt RETYPE from slot 1 (private untyped in A's CSpace).
    let (rc, _, _, _) = ecall(SLOT_PRIV_UT, UNTYPED_RETYPE,
                               CAPTYPE_ENDPOINT, 0, SLOT_NEW_EP, 1);
    if rc == 0 {
        print(b"[A] retype: OK (private untyped in slot 1)\n");
    } else {
        print(b"[A] retype: UNEXPECTED ERROR\n");
    }
    exit();
}
